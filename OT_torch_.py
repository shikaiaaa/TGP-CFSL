...
def SID(x,y):
    # print(np.sum(x))
	x = x.div(x.sum(2).unsqueeze(2))
	y = y.div(y.sum(2).unsqueeze(2))
	x_1 = torch.mul(x,torch.log(torch.div(x + 1e-12,y + 1e-12)))
	y_1 = torch.mul(y,torch.log(torch.div(y + 1e-12,x + 1e-12)))
	sid = x_1 + y_1
	cos_sid = torch.bmm(torch.transpose(sid, 1, 2), sid)
	return cos_sid
