...
class AttentionalGNN(nn.Module):
    def __init__(self, num_support, feature_dim: int, layer_names: list):
        ...
        self.linear1 = nn.Sequential(nn.Linear(in_features=self.emb_size,
                                               out_features=self.emb_size,
                                               bias=True),
                                     nn.BatchNorm1d(self.emb_size))
        self.linear2 = nn.Sequential(nn.Linear(in_features=self.emb_size,
                                               out_features=self.emb_size,
                                               bias=True),
                                     nn.BatchNorm1d(self.emb_size))
        self.linear3 = nn.Sequential(nn.Linear(in_features=self.emb_size,
                                               out_features=self.emb_size,
                                               bias=True),
                                     nn.BatchNorm1d(self.emb_size))
        self.linear4 = nn.Sequential(nn.Linear(in_features=self.emb_size,
                                               out_features=self.emb_size,
                                               bias=True),
                                     nn.BatchNorm1d(self.emb_size))
        self.linear5 = nn.Sequential(nn.Linear(in_features=self.emb_size,
                                               out_features=self.emb_size,
                                               bias=True),
                                     nn.BatchNorm1d(self.emb_size))
        self.linear6 = nn.Sequential(nn.Linear(in_features=self.emb_size,
                                               out_features=self.emb_size,
                                               bias=True),
                                     nn.BatchNorm1d(self.emb_size))
    ...
    def forward(self, p_nodes_src, p_nodes_tar, dis_nodes_src, dis_nodes_tar):
        ...
        for layer, name in zip(self.layers, self.names):
            if name == 'cross':
                p_tar, p_src = p_nodes_tar[flag], p_nodes_src[flag]
                d_tar, d_src = dis_nodes_tar[flag], dis_nodes_src[flag]
            ...
            x = self.linear1(p_nodes_src[flag].squeeze(0)).unsqueeze(0)
            temp1 = self.linear2(p_tar.squeeze(0)).unsqueeze(0)
            temp2 = self.linear3(p_tar.squeeze(0)).unsqueeze(0)
            y = self.linear4(p_nodes_tar[flag].squeeze(0)).unsqueeze(0)
            temp3 = self.linear5(p_src.squeeze(0)).unsqueeze(0)
            temp4 = self.linear6(p_src.squeeze(0)).unsqueeze(0)
            delta0, delta1 = layer(x, temp1, temp2), layer(y, temp3, temp4)
            p_nodes_src_temp, p_nodes_tar_temp = torch.einsum('bij,bij->bij', delta0, p_tar), torch.einsum(
                'bij,bij->bij', delta1, p_src)

            delta0, delta1 = layer(dis_nodes_src[flag], d_tar, p_nodes_src_temp), layer(dis_nodes_tar[flag], d_src,
                                                                                         p_nodes_tar_temp)
